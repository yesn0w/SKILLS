...
（`~/.codex/skills`）、`SKILL.md` 元数据、`$snow-NN-skill-name` 触发方式，以及
...
- `snow-01-bilingual-repo-docs`：维护英文和 `zh-CN` 仓库文档配对。
- `snow-02-investigate-repo`：改代码前调查仓库行为和相关证据。
- `snow-03-pr-prep`：检查仓库状态并准备干净的 PR 工作流。
- `snow-04-latest-origin-main`：同步到干净的最新 `origin/main`。

## 命名规则

Codex skill 包目录和 `SKILL.md` 的 `name` 值使用：
